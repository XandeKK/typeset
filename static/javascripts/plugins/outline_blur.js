function outline_blur(textbox, id=null) {
    let loaded = true;
    if (!id) {
        loaded = false;
        id = uuidv4();
        ...
        function: function (obj, arg) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                for (let j = 0; j < line.length; j++) {
                    ...
                    arg.ctx.shadowColor = obj[arg.id + '-fill'];
                    arg.ctx.shadowBlur = obj[arg.id + '-blur'];
                    arg.ctx.lineWidth = obj[arg.id + '-size'];
                    arg.ctx.strokeText(letter, x, y);
                    arg.ctx.shadowBlur = 0;
    ...
    textbox.textCanvas.addPlugin('outline_blur', {id: id, frontend: frontend, delete: _delete});
    if (!loaded) {
        return {id: id, name: 'outline_blur', properties: [id + '-size', id + '-fill', id + '-blur']};
    }
}